...
class CircutManipulation(SingleArmEnv):
    def __init__(
            self,
            robots,
    ...
    def _load_model(self):
        ...
        self.robots[0].robot_model.set_base_xpos([-0.6, 0, 0])
        ...
        # self.door._set_door_range([0, 1.5])
        ...
        self.cabinet_object = CabinetObject(
            name="CabinetObject")
        # cabinet_object = self.cabinet_object.get_obj(); cabinet_object.set("pos", array_to_string(cabinet_pos));
        ...
        Hammer_pos = np.array([0.5, 0.3, 1.2])
        hammer = self.hammer.get_obj()
        hammer.set('pos', array_to_string(Hammer_pos))
        hammer.set('quat', '1 0 0 0')

        self.desk_surface = Desk_Surface(name="desk_surface")
        desk_surface_object = self.desk_surface.get_obj()
        desk_surface_object.set("pos", '0.0 -0.1 0.95')
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.door, self.cabinet_object] ##把还没appended的object包含在内
        )

        self.model.worldbody.append(
            hammer)  ##如果把hammer作为table的一部分来看待的话，会产生“Error: free joint can only be used on top level”的报错，而作为worldbody的一部分的话不会产生body这个问题
        self.model.worldbody.append((desk_surface_object))

        ##所有有texture,mesh登asset的object都要merge_assets
        self.model.merge_assets(self.hammer)
        self.model.merge_assets(self.cabinet_object)
        self.model.merge_assets(self.door)
        self.model.merge_assets(self.desk_surface)
