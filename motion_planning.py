...
class Motion_planning():
    ...
    def range_box_G(self, start, target, start_range=np.array([0.04, 0.04, 0.02, 0.02]), target_range=np.array(
        [0.035, 0.035, 0.035])):  ##新的体素划分方式，考虑到夹爪张开时需要避让目标处的障碍物（如果有的话）；夹爪关闭则不用考虑避让目标处的障碍物(cabient(0.03, 0.03, 0.02))
        point1 = start
        obstacle1 = self.env.door_pos  ##障碍物：门的中心点
        obstacle2 = self.env._cabinet_base_center_xpos  ##障碍物：抽屉的中心点
        point2 = target
        obstacle1_box = np.array([0.025, 0.03, 0.06])  ##后面改成自动获取障碍物尺寸信息
        obstacle2_box = np.array([0.22, 0.22, 0.13])
        gripper_mini = np.array(
        ...
        self.target_range = [int(target_range[0] / self.dx), int(target_range[1] / self.dy),
                             int(target_range[2] / self.dz)]
        self.height = int((self.max_z + start_range[2] + start_range[3] - self.min_z) / self.dz) + 1
        self.length = int((self.max_x + 2 * start_range[0] - self.min_x) / self.dx) + 1
        self.width = int((self.max_y + 2 * start_range[1] - self.min_y) / self.dy) + 1
        self.Map = np.zeros(shape=(self.length, self.width, self.height))  ##创建了一个三维地图，为全空
        ...
        if self.gripper_open:  ##如果夹爪为打开状态，那么需要检测目标点附近的障碍；如果夹爪为关闭状态，则不需要考虑目标点附近的障碍
            ...
            self.end_point = [int((point2[0] - self.min_x) / self.dx), int((point2[1] - self.min_y) / self.dy),
                              int((point2[2] + target_range[2] + start_range[
                                  3] - self.min_z) / self.dz)]  ##结束点应为目标点向上平移
    ...
    def path_searching(self, start, end, joint_type=None, P_joint=np.array([0, 0, 0]),
        ...
        self.Astar = A_Search(self.start_point, self.end_point, self.Map, self.gripper_range,
                              gri_open=self.gripper_open, joint_type=joint_type,
    ...
    def path_smoothing(self, Path_points, t_final, freq):
        ...
        for i in range(self.n):
            ...
            self.d[i] = np.array([self.x_new[i], self.y_new[i], self.z_new[i]])
            if i == 0:
                self.d_dot[i] = np.array([0, 0, 0])
                self.d_ddot[i] = np.array([0, 0, 0])
            else:
                x_d = (self.x_new[i] - self.x_new[i - 1]) * freq
                y_d = (self.y_new[i] - self.y_new[i - 1]) * freq
                z_d = (self.z_new[i] - self.z_new[i - 1]) * freq
...
class A_Search:  # 核心部分，寻路类
    def __init__(self, arg_start, arg_end, arg_map, gri_range, P_joint=np.array([0, 0, 0]), gri_open=True,
                 joint_type=None, axis_joint=np.array([0, 0, 0])):  ##env为仿真环境；arg_start
        self.start = point(arg_start)  # 储存此次搜索的开始点
        self.end = point(arg_end)  # 储存此次搜索的目的点
        self.Map = arg_map  # 一个三维数组，为此次搜索的地图引用
        self.gri_open = gri_open  ##夹爪开关状态
        self.gri_range = gri_range
        self.joint_type = joint_type  ##关节类型
                        ...
                        if self.gri_open == True:
                            ...
                            if flag == 0:
                                if (self.Map[x - i][y][z] + self.Map[x][y - j][z] + self.Map[x][y][
                                    ...
                                    flag = 1
                                    print("the (%d,%d,%d) is not reached" % (x, y, z))
                                if self.joint_type == "hinge" and np.linalg.norm(
                                        np.dot((P_n - P_i), self.axis_joint)) != 0:
                                    flag = 1
                                    print("the (%d,%d,%d) break constraints" % (x, y, z))
                                if self.joint_type == "hinge" and abs(
                                        np.linalg.norm(P_n - self.P_joint) - np.linalg.norm(P_i - self.P_joint)) > 0.5:
                                    flag = 1
                                    print("the (%d,%d,%d) break constraints" % (x, y, z))
                                if self.joint_type == "slide" and np.linalg.norm(np.cross((P_n - P_i), self.axis_joint)) != 0:
                                    flag = 1
                                    print("the (%d,%d,%d) break constraints" % (x, y, z))
    ...
    def getGcost_extra(self, joint_type, P_n, P_i, P_joint, joint_axis):  ##施加约束代价函数的方式
        if joint_type == "slide":
            cost_extra = 100 * (np.linalg.norm(np.cross((P_n - P_i), joint_axis)))  ##对不与关节方向平行的运动施加约束
        elif joint_type == "hinge":
            cost_extra = 100 * (np.linalg.norm(np.dot((P_n - P_i), joint_axis)))  ##对不与关节方向垂直的运动施加约束
            cost_extra += 1000 * (
                abs(np.linalg.norm(P_n - P_joint) - np.linalg.norm(P_i - P_joint)))  ##对与关节距离改变的运动施加约束
        else:
            cost_extra = 0
...
if __name__ == "__main__":
    ...
    M = Motion_planning(env=env, dx=0.005, dy=0.005, dz=0.005, gripper_open=False)
    ##生成地图尝试进行搜索
    start = env._handle_xpos
    P_joint = env._hinge_joint_xpos
    # 将角度转换为弧度
    alpha_rad = math.radians(60)  ##开门的旋转角度为30度
    # 计算圆心到起始点的向量
    ...
    Points_recover = M.path_searching(start=start, end=end, joint_type="hinge",
