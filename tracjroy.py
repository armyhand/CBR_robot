...
class trac_plan:
    ...
    def virtual_trac(self):
        for i in range(self.n + 1):
            ...
            self.d[i] = self.d_init + (self.d_final - self.d_init) * (10 * (self.t[i] / self.t_final) ** 3 - 15 *
                                                                      (self.t[i] / self.t_final) ** 4 + 6 *
                                                                      (self.t[i] / self.t_final) ** 5)
            self.d_dot[i] = (self.d_final - self.d_init) * (30 * (self.t[i] / self.t_final) ** 2 - 60 *
                                                            (self.t[i] / self.t_final) ** 3 + 30 *
                                                            (self.t[i] / self.t_final) ** 4) * (1 / self.t_final)
            self.d_ddot[i] = (self.d_final - self.d_init) * (60 * (self.t[i] / self.t_final) - 180 *
    ...
    def DMP_trac_soft(self, x_trac):
        self.t = np.arange(0, self.t_final, 1/self.freq)
        xd0 = np.zeros([x_trac.shape[0], 1])
        xd = np.append(xd0, np.diff(x_trac), axis=1)
        xdd0 = np.zeros([x_trac.shape[0], 1])
        xdd = np.append(xdd0, np.diff(xd), axis=1)
        dmps = dmp.DMPs(start=x_trac[:, 0], goal=x_trac[:, -1] * 2, tau=1, n_bfs=100, end_dmp_time=2.0, end_time=2.2,
                    n_dmps=2)
    ...
    def R(self, k_x, k_y, k_z, theta):
        ...
        c = np.cos(theta)
        v = 1 - np.cos(theta)
        T1 = [(k_x * k_x) * v + c, k_x * k_y * v - k_z * s, k_x * k_z * v + k_y * s, 0]
        T2 = [k_x * k_y * v + k_z * s, k_y * k_y * v + c, k_y * k_z * v - k_x * s, 0]
        T3 = [k_x * k_z * v - k_y * s, k_y * k_z * v + k_x * s, (k_z * k_z) * v + c, 0]
        T4 = [0, 0, 0, 1]
    ...
    def resize(self, d_init_rec, d_fin_rec):  # omiga是轨迹的所放量
        ...
        for i in range(len(self.t)):
            ...
            for j in range(len(self.d[0])):
                ...
                self.d_ddot_ref[i][j] = omiga[j] * self.d_ddot[i][j] / (omiga_t ** 2)
            if i == len(self.t) - 1:
                break
            else:
                for j in range(len(self.d[0])):
                    self.d_ref[i + 1][j] = self.d_ref[i][j] + omiga[j] * (self.d[i + 1][j] - self.d[i][j])
        ...
        for j in range(len(self.d[0])):
            x = self.d_ref[:, j]
            z = np.polyfit(self.t_ref, x, 5)  # 使用五次多项式进行拟合，拟合精度还有待提高，对周期函数的拟合无能为力
            y = np.poly1d(z)
            x1 = y(self.t_res)
            self.d_res[:, j] = x1
        ...
        if np.linalg.norm(l1) != 0 and np.linalg.norm(l2) != 0:
            ...
            theta = np.arccos(vector_dot_product / (np.linalg.norm(l1) * np.linalg.norm(l2)))  # 求解新老方向向量夹角

            self.d_new = np.column_stack((self.d_res, np.ones([len(self.t_res), 1])))  # 增加一列1变成增广矩阵
            rot = self.rotate_mat(k_norm, theta)  # 或者用self.R代替，两者计算结果是一样的
            W = self.T(self.d_res[0, 0] + dl[0], self.d_res[0, 1] + dl[1], self.d_res[0, 2] + dl[2]) @ rot @ self.T(
                -self.d_res[0, 0], -self.d_res[0, 1], -self.d_res[0, 2]) #计算坐标转换矩阵，由两个平移矩阵和一个旋转矩阵相乘构成
            ...
            for j in range(len(self.d_init)):
                ...
                self.d_ddot_new[:, j] = np.append(0, np.diff(self.d_dot_new[:, j]))
        else:
